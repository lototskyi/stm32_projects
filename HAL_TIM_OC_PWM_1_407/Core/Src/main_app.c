...
void Timer2_Init(void)
{
	...
	timer2PWM_Config.OCMode = TIM_OCMODE_PWM2;
	...
	timer2PWM_Config.Pulse = (htimer2.Init.Period * 45) / 100;
	if (HAL_TIM_PWM_ConfigChannel(&htimer2, &timer2PWM_Config, TIM_CHANNEL_2) != HAL_OK) {
		Error_handler();
	}

	timer2PWM_Config.Pulse = (htimer2.Init.Period * 75) / 100;
	if (HAL_TIM_PWM_ConfigChannel(&htimer2, &timer2PWM_Config, TIM_CHANNEL_3) != HAL_OK) {
		Error_handler();
	}

	timer2PWM_Config.Pulse = (htimer2.Init.Period * 95) / 100;
