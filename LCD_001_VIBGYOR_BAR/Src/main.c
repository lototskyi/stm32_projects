...
int main(void)
{
	SystemClock_Setup();
	BSP_LCD_Init();
	BSP_LCD_Set_Orientation(BSP_LCD_ORIENTATION);
	LTDC_Pin_Init();
	LTDC_Init();
...
void LTDC_Init(void)
{
	...
	//default polarity for hsync, vsync, ltdc_clock, DE
...
void LTDC_Layer_Init(LTDC_Layer_TypeDef *pLayer)
{
	...
	uint32_t AHBP = (REG_READ(pLTDC->BPCR) >> LTDC_BPCR_AHBP_Pos) & 0xFFFU;
	...
	//6. configure pitch, line length and line numbers of the frame buffer
	tmp = 0;
	...
	REG_SET_VAL(pLayer->CFBLNR, BSP_LCD_ACTIVE_HEIGHT, 0x7FFU, LTDC_LxCFBLNR_CFBLNBR_Pos);
...
//	REG_SET_VAL(pGpioA->MODER, 0x02, 0x03, GPIO_MODER_MODE8_Pos);
//	REG_SET_VAL(pGpioA->AFR[1], 0x00, 0x0F, GPIO_AFRH_AFSEL8_Pos);
//	REG_SET_VAL(pGpioA->OSPEEDR, 0x01, 0x03, GPIO_OSPEEDR_OSPEED8_Pos);
//	REG_SET_VAL(pRCC->CFGR, 0x03, 0x03, RCC_CFGR_MCO1_Pos);
//	REG_SET_VAL(pRCC->CFGR, 0x07, 0x07, RCC_CFGR_MCO1PRE_Pos);
	...
	REG_SET_BIT(pRCC->PLLCFGR, RCC_PLLCFGR_PLLSRC_Pos); //source HSE
	REG_SET_VAL(pRCC->PLLCFGR, 0x04U, 0x3FU,  RCC_PLLCFGR_PLLM_Pos); //PLL_M
	...
	//5. Setting up AHB1, APB1 and APB2 clocks (APB2 = 90 MHz, APB1 = 45MHz)
	REG_SET_VAL(pRCC->CFGR, 0x00U, 0x0FU, RCC_CFGR_HPRE_Pos);
	REG_SET_VAL(pRCC->CFGR, 0x05U, 0x07U, RCC_CFGR_PPRE1_Pos); //div by 4
	REG_SET_VAL(pRCC->CFGR, 0x04U, 0x07U, RCC_CFGR_PPRE2_Pos); //div by 2
	...
	while (!(REG_READ_VAL(pRCC->CFGR, 0x03U, RCC_CFGR_SWS_Pos) == 0x02));
}
