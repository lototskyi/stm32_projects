...
void LCD_Pin_Init(void)
{
	...
	REG_SET_VAL(pGPIOA->OSPEEDR, 0x2U, 0x3, (LCD_RESX_PIN * 2U)); /*Speed*/

	//CSX
	REG_SET_VAL(pGPIOC->MODER, 0x1U, 0x3, (LCD_CSX_PIN * 2U)); /*mode*/
	REG_CLR_BIT(pGPIOC->OTYPER, LCD_CSX_PIN); /*Output type*/
	REG_SET_VAL(pGPIOC->OSPEEDR, 0x2U, 0x3, (LCD_CSX_PIN * 2U)); /*Speed*/

	//D/CX
	REG_SET_VAL(pGPIOD->MODER, 0x1U, 0x3, (LCD_DCX_PIN * 2U)); /*mode*/
	REG_CLR_BIT(pGPIOD->OTYPER, LCD_DCX_PIN); /*Output type*/
	REG_SET_VAL(pGPIOD->OSPEEDR, 0x2U, 0x3, (LCD_DCX_PIN * 2U)); /*Speed*/

	//SPI_CLK(PF7)
	REG_SET_VAL(pGPIOF->MODER, 0x2U, 0x3, (LCD_SCL_PIN * 2U)); /*alt mode*/
	REG_CLR_BIT(pGPIOF->OTYPER, LCD_SCL_PIN); /*Output type*/
	REG_SET_VAL(pGPIOF->OSPEEDR, 0x2U, 0x3, (LCD_SCL_PIN * 2U)); /*Speed*/
	REG_SET_VAL(pGPIOF->AFR[0], 0x5U, 0xFU, (LCD_SCL_PIN * 4U));

	//SPI_SDA(PF9)
	REG_SET_VAL(pGPIOF->MODER, 0x2U, 0x3, (LCD_SDA_PIN * 2U)); /*alt mode*/
	REG_CLR_BIT(pGPIOF->OTYPER, LCD_SDA_PIN); /*Output type*/
	REG_SET_VAL(pGPIOF->OSPEEDR, 0x2U, 0x3, (LCD_SDA_PIN * 2U)); /*Speed*/
	REG_SET_VAL(pGPIOF->AFR[1], 0x5U, 0xFU, ((LCD_SDA_PIN % 8) * 4U));

	REG_SET_BIT(pGPIOC->ODR, LCD_CSX_PIN);//CSX HIGH
	REG_SET_BIT(pGPIOA->ODR, LCD_RESX_PIN);//REX HIGH
	REG_SET_BIT(pGPIOD->ODR, LCD_DCX_PIN);//D/CX HIGH
}
...
void LCD_SPI_Init(void)
{
	...
	REG_SET_VAL(pSPI->CR1, 0x3U, 0x7U, SPI_CR1_BR_Pos); /* SPI clk = 90MHz/16 => 5.625MHz */
	REG_CLR_BIT(pSPI->CR1, SPI_CR1_CPOL_Pos); /* CPOL = 0 (CK to 0 when idle) */
	REG_CLR_BIT(pSPI->CR1, SPI_CR1_CPHA_Pos); /* CPHA = 0 (SPI mode 0, The first clock transition is the first data capture edge) */
